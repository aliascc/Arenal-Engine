...
XEResult Logger::ActivateLogToFile()
{
	std::lock_guard<std::mutex> lock(m_LogMutex);
	...
	XETODO("Remove this from here and replace");
...
XEResult Logger::DeactivateLogToFile()
{
	std::lock_guard<std::mutex> lock(m_LogMutex);
...
XEResult Logger::SetLogFilename(const std::wstring& path)
{
	std::lock_guard<std::mutex> lock(m_LogMutex);
...
void Logger::SetScriptXELogArrayType(asIObjectType* scriptXELogArrayType)
{
	std::lock_guard<std::mutex> lock(m_LogMutex);

	m_ScriptXELogArrayType = scriptXELogArrayType;
}
...
void Logger::AddNewLog(LogLevel logLevel, const std::wstring& msg)
{
	...
	if(m_LogLevel >= logLevel && logLevel != LogLevel::None)
	{
		...
		if(msg.compare(m_LastLog.m_Log) == 0)
		{
			...
			if(m_LastLog.m_ElapsedTimeSameLogPrint >= XE_LOG_DUPLICATE_TIMEOUT || tempElapsedTime >= XE_LOG_DUPLICATE_TIME)
			{
				printSameLog = true;
			}
			else
...
void Logger::InsertLogNoLock(LogLevel logLevel, const std::wstring& msg, bool countEqual)
{
	...
	XE_Base::GetDate(newLog.m_TimeStamp);
...
XEResult Logger::SaveLogsInFile()
{
	std::lock_guard<std::mutex> lock(m_LogMutex);
...
CScriptArray* Logger::GetLogsScript()
{
	...
	XEAssert(m_ScriptXELogArrayType != nullptr);
	if (m_ScriptXELogArrayType == nullptr)
	{
		return nullptr;
	}

	CScriptArray* asXELogArr = CScriptArray::Create(m_ScriptXELogArrayType);

	asXELogArr->Resize((int32_t)m_Logs.size());

	auto it = m_Logs.cbegin();
	auto itEnd = m_Logs.cend();
	for (uint32_t i = 0; it != itEnd; ++it, ++i)
	{
		XELog* log = reinterpret_cast<XELog*>(asXELogArr->At(i));

		XEAssert(log != nullptr);
		*log = *it;
	}
...
void Logger::MonitorRepeatedLogTimeOut()
{
	while(m_IsRunning)
	{
		std::this_thread::sleep_for(std::chrono::seconds(XE_LOG_TIMEOUT_THREAD_SLEEP));
		...
		if(m_LastLog.m_NumSameLogs >= 2)
		{
			...
			if(m_LastLog.m_ElapsedTimeSameLogPrint >= XE_LOG_DUPLICATE_TIMEOUT)
			{
				std::wstring sameLogMsg = L"";
