/** \file 
 * Contains definitions for basic use of the XEngine 
 */
...
//taking place
//#define XE_DEBUG_DEL_PRINT_VAR_NAME
//#define XE_DEBUG_REL_PRINT_VAR_NAME

#define _XEWSTR(x) L#x
#define _XESTR(x) #x 
#define XEWSTR(x) _XEWSTR(x) 
#define XESTR(x) _XESTR(x) 
...
/// </remarks>
#define XETODO(x) __pragma(message(__FILE__ "(" XESTR(__LINE__) "): TODO: " _XESTR(x))) 

/// <summary>
/// Release XE Engine Game Resources
/// Even if it is identical to ReleaseCOM
/// we want to have our own if we need to
/// change it in the future
/// </summary>
#define XERelease(x)	if(x != nullptr)	\
						{					\
							x->Release();	\
							x = nullptr;	\
						}
...
/// </summary>
#if defined(_DEBUG) && defined(XE_DEBUG_REL_PRINT_VAR_NAME)
#define ReleaseCOM(x)	if(x != nullptr)																				\
						{																								\
							...
							std::wstring mvName = std::wstring(L"Release DirectX Var: ") + std::wstring(XEWSTR(x));		\
...
/// </remarks>
#if defined(_DEBUG) && defined(XE_DEBUG_DEL_PRINT_VAR_NAME)
#define DeleteMem(x)	if(x != nullptr)																			\
						{																							\
							...
							std::wstring file(__FILEW__);															\
							std::wstring line(XEWSTR(__LINE__));													\
							std::wstring vname = std::wstring(L"Deleting Pointer: ") + std::wstring(XEWSTR(x));		\
...
#define DeleteMemArr(x)	if(x != nullptr)																				\
						{																								\
							...
							std::wstring file(__FILEW__);																\
							std::wstring line(XEWSTR(__LINE__));														\
							std::wstring vname = std::wstring(L"Deleting Pointer Array: ") + std::wstring(XEWSTR(x));	\
							std::wstring output = file + L"(" + line + L"): " + vname + L"\n";							\
							OutputDebugString(output.c_str());															\
						}
#else
...
/// </summary>
#define XEFloat4Align __declspec(align(16))

/// <summary>
/// Use to align a variable to 4 byte Boundaries
/// </summary>
#define XEByte4Align __declspec(align(4))
...
/// <summary>
/// XEngine Return Code Types
/// </summary>
enum class XEResult : int32_t
{
	/// <summary>
	/// Generic Failure
	/// </summary>
	...
	*   - GetXEResultString Function (BaseFunctions.cpp)
	*****************************************************/
