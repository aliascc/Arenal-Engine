...
XEResult StructuredBuffer::Deinitialize()
{
	m_IsReady		= false;
	m_NumElements	= 0;
	m_StructSize	= 0;
...
XEResult StructuredBuffer::InitializeBuffer(uint32_t structSize, uint32_t numElements, GraphicBufferUsage bufferUsage, GraphicBufferAccess bufferAccess)
{
	...
	*2. Create VRAM Structured Buffer
	...
	if(hr != S_OK)
	{
		...
		return XEResult::StructuredBufferInitFailed;
	}

	XEGraphicHelpers::SetDebugObjectName<ID3D11Buffer>(m_BufferDX, XE_DEBUG_SB_NAME_PREFIX + XE_Base::WideStr2String(m_Name));
	...
	*3. Create SRV for Structured Buffer
	...
	XEGraphicHelpers::SetDebugObjectName<ID3D11ShaderResourceView>(m_BufferSRVDX, XE_DEBUG_SB_SRV_NAME_PREFIX + XE_Base::WideStr2String(m_Name));
	...
	*4. Create UAV for Buffer if flag is set
	...
	if(m_IsRWEnabled)
	{
		...
		XEGraphicHelpers::SetDebugObjectName<ID3D11ShaderResourceView>(m_BufferSRVDX, XE_DEBUG_SB_UAV_NAME_PREFIX + XE_Base::WideStr2String(m_Name));
	}
...
XEResult StructuredBuffer::UpdateBuffer(const void* data, uint32_t elementCount, uint32_t startIndex, uint32_t offset, GraphicResourceMap resourceMap)
{
	if(!m_IsReady)
	{
