...
#pragma region Private Variables
		...
		Color m_BorderColor = XEColors::White;
...
#pragma region Framework Methods
		...
		XEResult Initialize(ShaderFilter shaderFilter = ShaderFilter::MinMagMipLinear, ShaderTextureAddressMode textureAddressModeU = ShaderTextureAddressMode::Clamp, ShaderTextureAddressMode textureAddressModeV = ShaderTextureAddressMode::Clamp, ShaderTextureAddressMode textureAddressModeW = ShaderTextureAddressMode::Clamp, float minLOD = -std::numeric_limits<float>::max(), float maxLOD = std::numeric_limits<float>::max(), float mipLODBias = 0.0f, uint32_t maxAnisotropy = 1, ShaderCompFunct compFunction = ShaderCompFunct::Never, const Color& borderColor = XEColors::White);

		/// <summary>
		/// Re-initialize Sampler with current object settings
		/// </summary>
		/// <returns>XEResult::Ok if successful</returns>
		XEResult Reinitialize();
