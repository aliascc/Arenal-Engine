...
XETODO("Check if we still need this, if we are generating meshes another way");
...
XEResult SphereShape::CreateSphere(uint32_t recursionLevels, const Color& color, bool clockWise)
{
	m_RecursionLevels = recursionLevels;
	m_Color = color;
	m_ClockWise = clockWise;
...
uint16_t SphereShape::AddVtx(VertexPositionColor& vtx)
{
	...
	std::wstring vtxStr = XEVertexHelper::GetVertexString(vtx);
	uint16_t idx = 0;
...
XEResult SphereShape::BuildSphere()
{
...
XEResult SphereShape::DrawSphere()
{
	if(!m_IsReady)
	{
