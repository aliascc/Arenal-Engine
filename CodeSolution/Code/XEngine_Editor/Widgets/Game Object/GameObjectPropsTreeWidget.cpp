...
*   Function Defs   *
*********************/
XETODO("Finish adding mutex for gameapp");
...
void GameObjectPropsTreeWidget::InitFields()
{
	...
	XEQTUserTemplateData<XEQTObjType>* GameObjectsTreeType = new XEQTUserTemplateData<XEQTObjType>(XEQTObjType::GameObjectsPropsTree);
	this->setUserData(XE_QT_USER_DATA_OBJ_TYPE_SLOT, GameObjectsTreeType);

	///////////////////////////////////////////
	//Set preferred indentation
	this->setIndentation(XE_QT_TREE_GAME_OBJECT_PROPS_INDENTATION);
...
XEResult GameObjectPropsTreeWidget::DisplayGameObjectInfo(GameObject* gameObject)
{
	if(!m_IsReady)
	{
...
XEResult GameObjectPropsTreeWidget::ReloadGameObjectInfo()
{
	if (!m_IsReady)
	{
		return XEResult::NotReady;
	...
	if (m_CurrentGameObject->HasMeshGOC())
	{
		...
		////////////////////////////////////////
		//Add Mesh GOC Widget
		XETODO("Change Asset manager for engine viewer");
