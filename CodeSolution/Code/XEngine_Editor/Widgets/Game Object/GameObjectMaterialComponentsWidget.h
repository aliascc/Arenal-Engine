...
class GameObjectMaterialComponentsWidget sealed : public QWidget
{
	...
	private:
		...
		XEResult DropAsset(QObject* object, ShaderType shaderType);

		XEResult SetGameAssetFromID(ShaderType shaderType, uint64_t gameAssetID);

		XEResult SetShaderGameAssetToCurrent(ShaderAsset* shaderAsset);
