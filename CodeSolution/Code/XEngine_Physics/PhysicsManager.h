...
class PhysicsManager sealed : public XEObject
{
	private:
...
#pragma region Private Methods
		...
		XEResult UpdatePhysicsActorObject3D();
...
#pragma region Framework Methods
		...
		XEResult Update(const TimerParams& timerParams);

		XEResult ConnectToPhysXDebugger(const std::wstring& ip = L"127.0.0.1", uint32_t port = 5425, uint32_t timeout = 100);

		XEResult DisconnectToPhysXDebugger();

		XEResult AddPhysicsActor(PhysicsActor* physicsActor);
