...
{
	memset(m_ToneMapTex, 0, sizeof(RenderTarget*) * XE_HDR_MAX_TONE_MAP_TEXS);
	memset(m_BloomTex, 0, sizeof(RenderTarget*) * XE_HDR_MAX_BLOOM_TEXS);
	memset(m_StartTexs, 0, sizeof(RenderTarget*) * XE_HDR_MAX_START_TEXS);
}
...
HDRPostProcess::~HDRPostProcess()
{
	...
	for(i = 0; i < XE_HDR_MAX_TONE_MAP_TEXS; ++i)
	{
		DeleteMem(m_ToneMapTex[i]);
	}

	for(i = 0; i < XE_HDR_MAX_BLOOM_TEXS; ++i)
	{
		DeleteMem(m_BloomTex[i]);
	}

	for(i = 0; i < XE_HDR_MAX_START_TEXS; ++i)
	{
...
void HDRPostProcess::LoadContent()
{
	...
	m_SceneRT->CreateRenderTargetTexture(m_GraphicDevice, "Scene RT", width, height, XE_POOL_DEFAULT, XE_PX_FMT_A16B16G16R16F);

	// Scaled version of the HDR scene texture
	m_SceneScaledRT->CreateRenderTargetTexture(m_GraphicDevice, "Scene Scaled", m_CropWidth / 4, m_CropHeight / 4, XE_POOL_DEFAULT, XE_PX_FMT_A16B16G16R16F);
	...
	m_BrightPass->CreateRenderTargetTexture(m_GraphicDevice, "Bright Pass", m_CropWidth / 4 + 2, m_CropHeight / 4 + 2, XE_POOL_DEFAULT, XE_PX_FMT_A8R8G8B8);

	// Create a texture to be used as the source for the star effect
	// Texture has a black border of single texel thickness to fake border 
	// addressing using clamp addressing
	m_StartSource->CreateRenderTargetTexture(m_GraphicDevice, "Star Source", m_CropWidth / 8 + 2, m_CropHeight / 8 + 2, XE_POOL_DEFAULT, XE_PX_FMT_A8R8G8B8);

	// Create a texture to be used as the source for the bloom effect
	// Texture has a black border of single texel thickness to fake border 
	// addressing using clamp addressing
	m_BloomSource->CreateRenderTargetTexture(m_GraphicDevice, "Bloom Source", m_CropWidth / 8 + 2, m_CropHeight / 8 + 2, XE_POOL_DEFAULT, XE_PX_FMT_A8R8G8B8);
	...
	XETODO("R16F can be R32F if device does not support R16F");
	m_AdaptedLuminanceCur->CreateRenderTargetTexture(m_GraphicDevice, "Adaptive Luminance Current", 1, 1, XE_POOL_DEFAULT, XE_PX_FMT_R16F);
	m_AdaptedLuminanceLast->CreateRenderTargetTexture(m_GraphicDevice, "Adaptive Luminance Last", 1, 1, XE_POOL_DEFAULT, XE_PX_FMT_R16F);
	...
	for(i = 0; i < XE_HDR_MAX_TONE_MAP_TEXS; ++i)
	{
		...
		XETODO("R16F can be R32F if device does not support R16F");
		m_ToneMapTex[i]->CreateRenderTargetTexture(m_GraphicDevice, "Tone Map", sampleLen, sampleLen, XE_POOL_DEFAULT, XE_PX_FMT_R16F);
	}

	// Create the temporary blooming effect textures
	// Texture has a black border of single texel thickness to fake border 
	// addressing using clamp addressing
	for(i = 1; i < XE_HDR_MAX_BLOOM_TEXS; ++i)
	{
		m_BloomTex[i]->CreateRenderTargetTexture(m_GraphicDevice, "Bloom Tex", m_CropWidth / 8 + 2, m_CropHeight / 8 + 2, XE_POOL_DEFAULT, XE_PX_FMT_A8R8G8B8);
	}

	//Create the final blooming effect texture
	m_BloomTex[0]->CreateRenderTargetTexture(m_GraphicDevice, "Final Bloom", m_CropWidth / 8, m_CropHeight / 8, XE_POOL_DEFAULT, XE_PX_FMT_A8R8G8B8);

	//Create the star effect textures
	for(i = 0; i < XE_HDR_MAX_START_TEXS; ++i)
	{
		m_StartTexs[i]->CreateRenderTargetTexture(m_GraphicDevice, "Start Texs", m_CropWidth / 4, m_CropHeight / 4, XE_POOL_DEFAULT, XE_PX_FMT_A16B16G16R16F);
	}
...
void HDRPostProcess::ClearTextures()
{
	...
	for(uint32_t i = 0; i < XE_HDR_MAX_BLOOM_TEXS; ++i)
	{
...
void HDRPostProcess::RenderFinalPass()
{
	...
	m_HDREffect->ActivateTechnique(HDRPostProcessMaterial::XE_HDR_FINAL_SCENE_PASS);
	...
	samplerState.m_MagFilter = XE_TF_POINT;
	samplerState.m_MinFilter = XE_TF_POINT;
	m_GraphicDevice->SetSamplerState(0, samplerState);
	m_GraphicDevice->SetSamplerState(3, samplerState);

	samplerState.m_MagFilter = XE_TF_LINEAR;
	samplerState.m_MinFilter = XE_TF_LINEAR;
...
void HDRPostProcess::RenderStar()
{
	...
	XMFLOAT4 sampleWeights[XE_HDR_MAX_SAMPLES];
	XMFLOAT2 sampleOffsets[XE_HDR_MAX_SAMPLES];

	XETODO("LOOK INTO THIS STATE CHANGE");
	static const GraphicBlendState blendState = GraphicBlendState(true, false, XE_BLENDOP_ADD, XE_BLEND_ONE, XE_BLEND_ONE);
	...
	for(d = 0; d < starDef.m_StarLineNum; ++d)
	{
		...
		for(p = 0; p < starLine.m_Passes; ++p)
		{
			...
			m_HDREffect->ActivateTechnique(HDRPostProcessMaterial::XE_HDR_STAR);
...
void HDRPostProcess::RenderBloom()
{
	...
	XMFLOAT4 sampleWeights[XE_HDR_MAX_SAMPLES];
	XMFLOAT2 sampleOffsets[XE_HDR_MAX_SAMPLES];
	float sampleOffsetsFloats[XE_HDR_MAX_SAMPLES];
	...
	m_HDREffect->ActivateTechnique(HDRPostProcessMaterial::XE_HDR_GUASS_BLUR_5x5);
	...
	for(uint32_t i = 0; i < XE_HDR_MAX_SAMPLES; ++i)
	{
		sampleOffsets[i] = XMFLOAT2(sampleOffsetsFloats[i], 0.0f);
	}

	m_HDREffect->ActivateTechnique(HDRPostProcessMaterial::XE_HDR_BLOOM);
	m_HDREffect->SetSampleOffsets(sampleOffsets);
	m_HDREffect->SetSampleWeights(sampleWeights);
	...
	for(uint32_t j = 0; j < XE_HDR_MAX_SAMPLES; ++j)
	{
...
void HDRPostProcess::StarSourceBloomSource()
{
	...
	m_HDREffect->ActivateTechnique(HDRPostProcessMaterial::XE_HDR_DOWN_SCALE_2x2);
	...
	samp.m_MinFilter = XE_TF_POINT;
	samp.m_AddressU = XE_TAM_CLAMP;
	samp.m_AddressV = XE_TAM_CLAMP;
...
{	
	...
	// bright reflections
	m_HDREffect->ActivateTechnique(HDRPostProcessMaterial::XE_HDR_BRIGHT_PASS_FILTER);
...
void HDRPostProcess::CalculateAdaptation(const TimerParams& timerParams)
{
	...
	// level.
	m_HDREffect->ActivateTechnique(HDRPostProcessMaterial::XE_HDR_CALCULATE_ADAPTED_LUM);
...
void HDRPostProcess::MeasureLuminance()
{
	...
	uint32_t curTexture = XE_HDR_MAX_TONE_MAP_TEXS - 1;
	...
	m_HDREffect->ActivateTechnique(HDRPostProcessMaterial::XE_HDR_SAMPLE_AVG_LUM);
	m_HDREffect->SetSampleOffsets(sampleOffsets);
	...
	samp.m_MagFilter = XE_TF_LINEAR;
	samp.m_MinFilter = XE_TF_LINEAR;
	m_GraphicDevice->SetSamplerState(0, samp);
	m_GraphicDevice->SetSamplerState(1, samp);
	...
	while( curTexture > 0 )
	{
		...
		m_HDREffect->ActivateTechnique(HDRPostProcessMaterial::XE_HDR_RESAMPLE_AVG_LUM);
		m_HDREffect->SetSampleOffsets(sampleOffsets);
	...
	m_HDREffect->ActivateTechnique(HDRPostProcessMaterial::XE_HDR_RESAMPLE_AVG_LUM_EXP);
	m_HDREffect->SetSampleOffsets(sampleOffsets);
...
void HDRPostProcess::ScaleScene()
{
	...
	m_HDREffect->ActivateTechnique(HDRPostProcessMaterial::XE_HDR_DOWN_SCALE_4x4);
...
XEResult HDRPostProcess::GetTextureRect(RenderTarget* rt, RECT& rect)
{
	assert(rt);
	...
	for(int32_t x = -2; x <= 2; x++ )
	{
		for(int32_t y = -2; y <= 2; y++ )
		{
			XETODO("Look into this")
				...
				XEMathHelperVector::MultiplyVector4(white, XE_Base::GaussianDistribution((float)x, (float)y, 1.0f), sampleWeight[index]);
...
void HDRPostProcess::GetSampleOffsetsBloom(uint32_t texSize, float texCoordOffset[15], XMFLOAT4 colorWeight[], float deviation, float multiplier)
{
	...
	float weight = multiplier * XE_Base::GaussianDistribution(0.0f, 0.0f, deviation);
	...
	for( i = 1; i < 8; i++ )
	{
		...
		weight = multiplier * XE_Base::GaussianDistribution((float)i, 0.0f, deviation );
