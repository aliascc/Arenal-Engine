...
XEResult FPRObjectDraw::RenderWithFPR()
{
	...
	if(gameObject->HasMeshGOC() && gameObject->HasMaterialGOCs())
	{
		...
		if (mesh != nullptr && currentCamera != nullptr)
		{
			...
			for(uint32_t i = 0; i < mesh->GetNumberMeshParts() && i < matList.size(); i++, matListIt++)
			{
				...
				{
					ConstantBuffer* cbVS = vsProps->GetConstantBuffer(XE_CB_WORLD_VIEW_PROJ_NAME);
					if (cbVS == nullptr)
					{
						cbVS = vsProps->GetConstantBuffer(XE_CB_VIEW_PROJ_NAME);
					}
					else
					{
						cbVS->SetValueT<glm::mat4>(XE_CB_WORLD_VAR_NAME, gameObject->GetWorldTransform());
					}

					if (cbVS != nullptr)
					{
						cbVS->SetValueT<glm::mat4>(XE_CB_VIEW_VAR_NAME, currentCamera->GetViewMatrix());
						cbVS->SetValueT<glm::mat4>(XE_CB_PROJECTION_VAR_NAME, currentCamera->GetProjectionMatrix());
					}
					...
					ConstantBuffer* cbPS = psProps->GetConstantBuffer(XE_CB_FPR_LIGHTS_NAME);
					if (cbPS != nullptr)
					{
						cbPS->SetValueT<glm::vec3>(XE_CB_CAMERA_POS_VAR_NAME, currentCamera->GetPosition());
						cbPS->SetValueT<uint32_t>(XE_CB_WINDOW_WIDTH_VAR_NAME, m_GraphicDevice->GetGraphicPP().m_BackBufferWidth);
					}

					/////////////////////////////////////////////////////////
					//Get Current Shader Variables
					SimpleBuffer* currentPerTileLightIndexBuffer		= psProps->GetSimpleBuffer(XE_BF_PER_TILE_LIGHT_INDEX_BUFFER_NAME);
					StructuredBuffer* currentLightBuffer				= psProps->GetStructuredBuffer(XE_BF_LIGHT_BUFFER_NAME);
					StructuredBuffer* currentShadowSpotLightInfoBuffer	= psProps->GetStructuredBuffer(XE_BF_SHADOW_SPOT_LIGHT_INFO_BUFFER_NAME);
					StructuredBuffer* currentShadowDirLightInfoBuffer	= psProps->GetStructuredBuffer(XE_BF_SHADOW_DIR_LIGHT_INFO_BUFFER_NAME);
					TextureArray* currentSpotLightShadowTextureArray	= psProps->GetTextureArray(XE_TXA_SPOT_LIGHT_SHADOW_TEXTURE_ARRAY_NAME);
					TextureArray* currentDirLightShadowTextureArray		= psProps->GetTextureArray(XE_TXA_DIR_LIGHT_SHADOW_TEXTURE_ARRAY_NAME);
					Sampler* currentShadowTextureSample					= psProps->GetSampler(XE_SAM_SHADOW_TEXTURE_SAMPLER_NAME);
					...
					{
						XETODO("Check Return");
						psProps->OverrideStructuredBuffer(XE_BF_LIGHT_BUFFER_NAME, forwardPlusLightBuffer);
					}
					...
					{
						XETODO("Check Return");
						psProps->OverrideSimpleBuffer(XE_BF_PER_TILE_LIGHT_INDEX_BUFFER_NAME, forwardPlusPerTileLightIndexBuffer, true, false, true, XE_BI_PER_TILE_LIGHT_INDEX_BUFFER_DRAW_BIND_IDX);
					}
					...
					{
						XETODO("Check Return");
						psProps->OverrideStructuredBuffer(XE_BF_SHADOW_SPOT_LIGHT_INFO_BUFFER_NAME, forwardShadowSpotLightInfo);
					}
					...
					{
						XETODO("Check Return");
						psProps->OverrideTextureArray(XE_TXA_SPOT_LIGHT_SHADOW_TEXTURE_ARRAY_NAME, forwardSpotLightShadowTextureArray);
					}
					...
					{
						XETODO("Check Return");
						psProps->OverrideStructuredBuffer(XE_BF_SHADOW_DIR_LIGHT_INFO_BUFFER_NAME, forwardShadowDirLightInfo);
					}
					...
					{
						XETODO("Check Return");
						psProps->OverrideTextureArray(XE_TXA_DIR_LIGHT_SHADOW_TEXTURE_ARRAY_NAME, forwardDirLightShadowTextureArray);
					}
					...
					{
						XETODO("Check Return");
						psProps->OverrideSampler(XE_SAM_SHADOW_TEXTURE_SAMPLER_NAME, forwardShadowTextureSampler);
					}
