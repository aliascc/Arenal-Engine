...
AEResult BaseAddOnAS::RegisterAEResultEnum(asIScriptEngine* engine)
{
    ...
    ret = engine->RegisterEnumValue("AEResult", "ConfigLoadError", static_cast<int32_t>(AEResult::ConfigLoadError));
    if (ret < 0)
    {
        return AEResult::RegEnumValFail;
    }

    ret = engine->RegisterEnumValue("AEResult", "ExceptionHandledFailed", static_cast<int32_t>(AEResult::ExceptionHandledFailed));
    if (ret < 0)
    {
        return AEResult::RegEnumValFail;
    }
