...
namespace XEGameContentHelpers
{
	...
	XEResult FileGameContentType(const std::wstring& file, GameContentType& contentType, GameContextFileExt& fileExt)
	{
		...
		std::wstring ext = XE_Base::GetFilenameExtension(file);

		if (ext.empty())
		{
			return XEResult::EmptyFileExtension;
		}

		boost::algorithm::to_lower(ext);

		if (ext.compare(XE_CT_MODEL_COLLADA_EXT) == 0)
		{
		...
		if (ext.compare(XE_CT_SHADER_HLSL_1_EXT) == 0)
		{
		...
		if (ext.compare(XE_CT_SHADER_HLSL_2_EXT) == 0)
		{
			contentType = GameContentType::Shader;
		...
		if (ext.compare(XE_CT_TEXTURE_DDS_EXT) == 0)
		{
		...
		if (ext.compare(XE_CT_GAMEOBJECT_SCRIPT_EXT) == 0)
		{
		...
		if (ext.compare(XE_CT_AUDIO_1_EXT) == 0)
		{
		...
		if (ext.compare(XE_CT_AUDIO_2_EXT) == 0)
		{
			contentType = GameContentType::Audio;
		...
		return XEResult::UnknownFileExtension;
	}
		
	XEResult FileShaderyType(const std::wstring& file, ShaderType& shaderType)
	{
		XEAssert(!file.empty());
		...
		if(ext.compare(XE_CT_SHADER_HLSL_VS_EXT) == 0)
		{
			shaderType = ShaderType::VertexShader;
		}
		else if(ext.compare(XE_CT_SHADER_HLSL_PS_EXT) == 0)
		{
			shaderType = ShaderType::PixelShader;
		}
		else if(ext.compare(XE_CT_SHADER_HLSL_GS_EXT) == 0)
		{
			shaderType = ShaderType::GeometryShader;
		}
		else if(ext.compare(XE_CT_SHADER_HLSL_DS_EXT) == 0)
		{
			shaderType = ShaderType::DomainShader;
		}
		else if(ext.compare(XE_CT_SHADER_HLSL_HS_EXT) == 0)
		{
			shaderType = ShaderType::HullShader;
		}
		else if(ext.compare(XE_CT_SHADER_HLSL_CS_EXT) == 0)
		{
