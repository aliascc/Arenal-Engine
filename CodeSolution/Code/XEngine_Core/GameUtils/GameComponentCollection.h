...
*   Class Decl   *
******************/
class GameComponentCollection : public XEObject
{
...
#pragma endregion
		...
		//Framework Methods
		XEResult			Add							(GameComponent* gc);
		XEResult			Remove						(GameComponent* gc);
		XEResult			SetGCDrawCallOrder			(uint32_t id, uint32_t drawCallOrder);
		XEResult			SetGCVisible				(uint32_t id, bool visible);
		XEResult			SetGCEnable					(uint32_t id, bool enable);
