...
XEResult DiffuseTextureMaterial::CreateVertexShader()
{
	XEResult ret = XEResult::Ok;

	/////////////////////////////////////////////////////
	//Get Vertex Shader from Game Resources
	m_VertexShader = (VertexShader*)m_GameResourceManager->AcquireGameResourceByStringID(XE_DIFFUSE_TEXTURE_MAT_VS_NAME, GameResourceType::VertexShader);

	if(m_VertexShader == nullptr)
	{
		m_VertexShader = new VertexShader(m_GraphicDevice, XE_DIFFUSE_TEXTURE_MAT_VS_NAME);
		...
		ret = m_GameResourceManager->ManageGameResource(m_VertexShader, XE_DIFFUSE_TEXTURE_MAT_VS_NAME);
		if(ret != XEResult::Ok)
		{
...
XEResult DiffuseTextureMaterial::CreateVertexShaderConstantBuffer()
{
	XEResult ret = XEResult::Ok;
...
XEResult DiffuseTextureMaterial::CreatePixelShader()
{
	XEResult ret = XEResult::Ok;
	
	/////////////////////////////////////////////////////
	//Get Pixel Shader from Game Resources
	m_PixelShader = (PixelShader*)m_GameResourceManager->AcquireGameResourceByStringID(XE_DIFFUSE_TEXTURE_MAT_PS_NAME, GameResourceType::PixelShader);
	
	if(m_PixelShader == nullptr)
	{
		m_PixelShader = new PixelShader(m_GraphicDevice, XE_DIFFUSE_TEXTURE_MAT_PS_NAME);
		...
		ret = m_GameResourceManager->ManageGameResource(m_PixelShader, XE_DIFFUSE_TEXTURE_MAT_PS_NAME);
		if(ret != XEResult::Ok)
		{
			return XEResult::ResourceManagedFailed;
		}
...
XEResult DiffuseTextureMaterial::CreatePixelShaderTextureBinding()
{
	XEResult ret = XEResult::Ok;
	...
	//as in Shader:
	// Texture2D _XE_DiffuseTexture : register(t2);
	//

	ShaderTextureBinding* stb = new ShaderTextureBinding(XE_TX_DIFFUSE_TEXTURE_NAME, 2, TextureType::Texture2D, nullptr);
...
XEResult DiffuseTextureMaterial::LoadContent()
{
