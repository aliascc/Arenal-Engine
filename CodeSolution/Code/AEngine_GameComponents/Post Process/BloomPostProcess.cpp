...
void BloomPostProcess::LoadContent()
{
	...
	m_SceneRT->CreateRenderTargetTexture(m_GraphicDevice, "Scene RT", width, height, AE_POOL_DEFAULT, AE_PX_FMT_A8R8G8B8);
...
{	
	...
	m_BloomEffect->ActivateTechnique(BloomPostProcessMaterial::AE_BLOOM_EXTRACT);
...
AEResult BloomPostProcess::RenderBloomGaussianBlur()
{
	uint32_t passes = 0;
	...
	m_BloomEffect->ActivateTechnique(BloomPostProcessMaterial::AE_BLOOM_GAUSSIAN_BLUR);
...
void BloomPostProcess::SetBlurParameters(float dx, float dy)
{
	...
	float sampleWeights[AE_BLOOM_BLUR_SAMPLE_COUNT];
	XMFLOAT2 sampleOffsets[AE_BLOOM_BLUR_SAMPLE_COUNT];

	// The first sample always has a zero offset.
	sampleWeights[0] = AE_Base::GaussianDistribution(0, 0, m_Settings.m_BlurAmount);// ComputeGaussianBlur(0);
	...
	for (int i = 0; i < AE_BLOOM_BLUR_SAMPLE_COUNT / 2; i++)
	{
		// Store weights for the positive and negative taps.
		float weight = AE_Base::GaussianDistribution(i + 1.0f, 0, m_Settings.m_BlurAmount);// ComputeGaussianBlur(i + 1.0f);
	...
	for (int i = 0; i < AE_BLOOM_BLUR_SAMPLE_COUNT; i++)
	{
...
AEResult BloomPostProcess::RenderBloomCombine()
{
	uint32_t passes = 0;
	
	m_BloomEffect->ActivateTechnique(BloomPostProcessMaterial::AE_BLOOM_COMBINE);
