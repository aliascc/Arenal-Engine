...
AEResult StarProp::Initialize(const std::string& name, uint32_t starLineNum, float inclination, bool rotation, uint32_t passes, float sampleLength, float attenuation)
{
	if(starLineNum == 0)
	{
...
AEResult StarPropSunny::Initialize(const std::string& name, float sampleLength, float attenuation, float longAttenuation, float inclination)
{
	CleanUp();
