...
void ParticleSystem::Initialize()
{
	...
	//Create the Vertex Buffer Instance, set usage as AE_USAGE_DYNAMIC to make the Vertex Buffer a Dynamic Buffer and 
	//lock to AE_LOCK_DISCARD to increase performance when writing to the VRAM
	m_VertexBuffer = VertexBuffer<VertexParticle>::CreateVertexBufferInst(m_GraphicDevice, AE_POOL_DEFAULT, AE_USAGE_WRITEONLY | AE_USAGE_DYNAMIC);
	...
	if (m_FirstActiveParticle != m_FirstFreeParticle)
	{
		...
		for(uint32_t i = 0; i < passes; ++i)
		{
			...
			if (m_FirstActiveParticle < m_FirstFreeParticle)
			{
				...
				m_GraphicDevice->DrawIndexedPrimitive(AE_PT_TRIANGLELIST, 0, 
...
AEResult ParticleSystem::AddParticle(const XMFLOAT3& position, const XMFLOAT3& velocity)
{
	...
	float horizontalVelocity = AE_Base::Lerp<float>(m_Settings.m_MinHorizontalVelocity, m_Settings.m_MaxHorizontalVelocity, AERandomHelpers::GetNextFloat());

	float horizontalAngle = AERandomHelpers::GetNextFloat() * XM_2PI;
	...
	newVelocity.y += AE_Base::Lerp<float>(m_Settings.m_MinVerticalVelocity, m_Settings.m_MaxVerticalVelocity, AERandomHelpers::GetNextFloat());

	// Choose four random control values. These will be used by the vertex
	// shader to give each particle a different size, rotation, and color.
	XMCOLOR randomValues(
				AERandomHelpers::GetNextFloat(),
				AERandomHelpers::GetNextFloat(),
				AERandomHelpers::GetNextFloat(),
				AERandomHelpers::GetNextFloat()
				);
