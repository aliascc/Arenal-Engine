...
* Input Resources
******************/
SamplerState _XE_ShadowTextureSampler	: register(s0);

StructuredBuffer<Light>					_XE_LightBuffer						: register(t0);
Buffer<uint>							_XE_PerTileLightIndexBuffer			: register(t1);
StructuredBuffer<SpotLightShadowInfo>	_XE_ShadowSpotLightInfoBuffer		: register(t2);
Texture2DArray							_XE_SpotLightShadowTextureArray		: register(t3);
Texture2DArray							_XE_DirLightShadowTextureArray		: register(t4);
...
LightOutput ForwardPlusLightCalculation(float3 positionWorldSpace, float3 normal, float specularFactor, float2 positionScreen, float depthView, float3 posVtx)
{
	...
	uint tileIndex = GetTileIndex(positionScreen, _XE_WindowWidth);

	uint lightIndex = (MAX_NUM_LIGHTS_PER_TILE + 1) * tileIndex;

	uint lightBufferIndex = _XE_PerTileLightIndexBuffer[lightIndex];
	...
	while(contLoopLights && lightBufferIndex != LIGHT_INDEX_BUFFER_SENTINEL)
	{
		...
		currentLight = _XE_LightBuffer[lightBufferIndex];
		...
		switch(currentLight.LightType)
		{
			case LIGHT_SPOT:
				...
				{
					shadowContrib = CalcShadowVarianceSpotLight(positionWorldSpace, currentLight.ShadowTextureIndex, _XE_ShadowSpotLightInfoBuffer, _XE_SpotLightShadowTextureArray, _XE_ShadowTextureSampler);
				}
				break;
			...
			case LIGHT_DIRECTIONAL:
				...
				if ((currentLight.LightFlags & LIGHT_FLAG_SHADOWENABLE) != 0)
				{
					shadowContrib = CalcShadowVarianceCascadeDirLightSimple(positionWorldSpace, depthView, currentLight.ShadowTextureIndex, _XE_ShadowDirLightInfoBuffer, _XE_DirLightShadowTextureArray, _XE_ShadowTextureSampler);
					//shadowContrib = CalcShadowVarianceCascadeDirLight(posVtx, depthView, currentLight.ShadowTextureIndex, _XE_ShadowDirLightInfoBuffer, _XE_DirLightShadowTextureArray, _XE_ShadowTextureSampler);
				}
				break;
		...
		if(contLoopLights)
		{
			...
			lightOutput.SpecularColor += CalculateSpecular(positionWorldSpace, specularFactor, _XE_CameraPos, normal, currentLight, speculatShineTemp);
			lightOutput.SpecularShine += speculatShineTemp;

			lightIndex++;
			lightBufferIndex = _XE_PerTileLightIndexBuffer[lightIndex];
		}
	}
