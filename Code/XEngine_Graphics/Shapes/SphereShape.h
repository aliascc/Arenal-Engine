...
*   Class Decl   *
******************/
class SphereShape sealed : public XEObject
{
	private:
		//Variables
	...
	public:
		...
		SphereShape(GraphicDevice* graphicDevice, uint32_t recursionLevels, const Color& color = XEColors::Red, bool clockWise = false);
		virtual ~SphereShape();
		...
		//Framework Methods
		XEResult							CreateSphere		(uint32_t recursionLevels, const Color& color = XEColors::Red, bool clockWise = false);
		XEResult							BuildSphere			();
		XEResult							DrawSphere			();
};
