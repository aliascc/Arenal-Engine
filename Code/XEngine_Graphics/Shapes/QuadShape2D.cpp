...
XEResult QuadShape2D::BuildQuad()
{
...
XEResult QuadShape2D::Resize(const RECT& size, const glm::vec4& textCoords)
{
	if (!m_IsReady)
	{
	...
	if(m_CurrentTexCoords == textCoords && XEMathHelpers::EqualRECT(m_CurrentSize, size))
	{
		return XEResult::Ok;
	}
	...
	if(m_VB->SetVertexData(0, m_Vtx, 0, 4, D3D11_MAP_WRITE_DISCARD) != XEResult::Ok)
	{
		return XEResult::Fail;
	}
