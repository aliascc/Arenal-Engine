...
void ViewerApp::Initialize()
{
	...
	//m_ViewerCamera = new ViewerCamera(m_InputManager, XE_CAMERA_EDITOR_ENG_DEFAULT_NAME, glm::vec3(0.0f, 0.0f, 5.0f), glm::vec3(0.0f, 0.0f, 0.0f), XEMathHelpers::Vec3fUp, dimesions, 45.0f/*glm::pi<float>() * .25f*/, 1.0f, 1000.0f);
	...
	//m_GameAssetManager->CreateNewRawGameAsset(XE_BASE_FS_PATH L".\\Data\\Raw Files\\Lobo.dae", XE_BASE_FS_PATH L".\\Data\\Assets\\", GameContentSubtype::None);
...
{
	XE_Base::ConsolePrintLine(L"Enter callback");
}
...
{
	/*
	if(m_InputHandler->KeyboardWasKeyPressed(XEKeys::O))
	{
		m_DebugStats->SetGridEnabled(!m_DebugStats->GetGridEnabled());
	}
	if(m_InputHandler->KeyboardWasKeyPressed(XEKeys::P))
	{
		m_DebugStats->SetAxisEnabled(!m_DebugStats->GetAxisEnabled());
	}
	if(m_InputHandler->KeyboardWasKeyPressed(XEKeys::I))
	{
		m_DebugStats->SetFPSEnabled(!m_DebugStats->GetFPSEnabled());
	}
	
	static glm::vec3 rotate = XEMathHelpers::Vec3fZero;
	static float scale = 1.0f;

	glm::mat4 w = XEMathHelpers::Mat4Identity;
	glm::mat4 wvp = XEMathHelpers::Mat4Identity;
	
	m_DiffuseTextureMaterial->GetVSProps()->GetConstantBuffer(L"_XE_CB_World_View_Proj")->SetValueT<glm::mat4>(L"_XE_World", w);
	m_DiffuseTextureMaterial->GetVSProps()->GetConstantBuffer(L"_XE_CB_World_View_Proj")->SetValueT<glm::mat4>(L"_XE_View", m_ViewerCamera->GetViewMatrix());
	m_DiffuseTextureMaterial->GetVSProps()->GetConstantBuffer(L"_XE_CB_World_View_Proj")->SetValueT<glm::mat4>(L"_XE_Projection", m_ViewerCamera->GetProjectionMatrix());

	m_SkinningMaterial->GetVSProps()->GetConstantBuffer(L"_XE_CB_World_View_Proj")->SetValueT<glm::mat4>(L"_XE_World", w);
	m_SkinningMaterial->GetVSProps()->GetConstantBuffer(L"_XE_CB_World_View_Proj")->SetValueT<glm::mat4>(L"_XE_View", m_ViewerCamera->GetViewMatrix());
	m_SkinningMaterial->GetVSProps()->GetConstantBuffer(L"_XE_CB_World_View_Proj")->SetValueT<glm::mat4>(L"_XE_Projection", m_ViewerCamera->GetProjectionMatrix());

	m_SkinningMaterial->GetVSProps()->GetConstantBuffer(L"_XE_CB_Bones")->SetValue(L"_XE_BoneTransforms", m_AnimationPlayer->GetBoneTransforms(), XE_MAX_BONES * sizeof(glm::mat4));*/
...
//
//			m_SkinningMaterial->GetPSProps()->GetShaderTextureBinding(L"_XE_DiffuseTexture")->SetTexture(txm->m_DiffuseTexture);
//
