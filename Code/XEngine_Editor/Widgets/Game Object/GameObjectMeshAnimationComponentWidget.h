...
class GameObjectMeshAnimationComponentWidget sealed : public QWidget
{
	...
	private:
		...
		XEResult AddGameAssetFromID(uint64_t gameAssetID);

		XEResult AddSkeletonAsset(SkeletonAsset* skeletonAsset);

		XEResult RemoveSkeletonAsset();

		XEResult AddAnimationAsset(AnimationAsset* animAsset);

		XEResult RemoveAnimationAsset(uint64_t assetID);
