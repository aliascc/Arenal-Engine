...
void GameObjectTransformationWidget::InitFields()
{
	...
	m_UniqueCallbackID = XE_Base::GetNextUniqueID();
	...
	XEResult ret = m_GameObject->AddObject3DChangeEventCallback(m_UniqueCallbackID, changeEvent);
	if (ret != XEResult::Ok)
	{
		XEAssert(false);
		XETODO("Log critical error");
	}
