	...
	if(gameObject->HasMeshGOC() && gameObject->HasMaterialGOCs())
	{
		...
		if(mesh != nullptr)
		{
			...
			for(uint32_t i = 0; i < mesh->GetNumberMeshParts() && i < matList.size(); i++, matListIt++)
			{
				...
				if(vsProps != nullptr)
				{
					ConstantBuffer* cb = vsProps->GetConstantBuffer(L"_XE_CB_World_View_Proj");

					cb->SetValueT<glm::mat4>(L"_XE_View", m_Camera->GetViewMatrix());
					cb->SetValueT<glm::mat4>(L"_XE_Projection", m_Camera->GetProjectionMatrix());
					cb->SetValueT<glm::mat4>(L"_XE_World", gameObject->GetWorldTransform());
				}
